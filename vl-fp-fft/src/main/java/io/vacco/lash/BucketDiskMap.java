...
import io.vacco.lash.util.RecordPtr;
import io.vacco.lash.util.Hash;
import io.vacco.lash.util.MMapper;
...
public class BucketDiskMap extends ADiskMap {
  ...
  public BucketDiskMap(File baseFolderLoc) {
    this(baseFolderLoc, 0);
  }

  public BucketDiskMap(File baseFolderLoc, long primaryFileLen) {
  ...
  private static List<List<RecordPtr>> splitToBuckets(List<RecordPtr> data) {
    final List<List<RecordPtr>> out = new ArrayList<>();
    if (data.size() == 0) {
  ...
  protected void rehashIdx(long idx) {
    ...
    final ArrayList<RecordPtr> keepBuckets = new ArrayList<>();
    final ArrayList<RecordPtr> moveBuckets = new ArrayList<>();
  ...
  public class BucketDiskMapEntry implements Entry<byte[], byte[]> {
    byte[] k, v;
    RecordPtr ptr;
