...
import io.vacco.volach.fprint.pk.dto.VlLocalPeak;
  ...
  static {
    ...
    it("It performs peak training from sample dataset", () -> {
      ...
      JtTrainer trainer = new JtTrainer(
          ...
          new VlAnchorTrainingSupplier(mapper, new File("./peak-training/peaks.json"))
      );
      trainer.start();
      ...
      Function<Integer, Boolean> inRange = (i -> i >= 10 && i <= 20); // TODO implement peak region merging: Map<Integer, Map<Double, VlAnchorPoint>>
      ...
      for (int j = 16; j < src.spectrum.length - 16; j += 16) {
        for (int k = 16; k < src.spectrum[0].length - 16; k += 4) {
          ...
          // && inRange.apply(lp.y)
          // TODO constrain location range on X only
          if (pt != null && lp.value > 0.09 && inRange.apply(lp.x)) {
